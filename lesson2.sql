...
select * from department join client on department.idDepartment = client.Department_idDepartment where DepartmentCity='kyiv';
...
select FirstName, count(FirstName) from client group by FirstName;

# 8. Вывести данные про клиентов, которые имеют кредиты больше чем 5000 гривен.
select * from application join volodymyrzp.client c on c.idClient = application.Client_idClient where Sum > 5000;
...
# 17. Всем клиентам, которые имеют высшее образование, установить все их кредити в размене 6000грн.
